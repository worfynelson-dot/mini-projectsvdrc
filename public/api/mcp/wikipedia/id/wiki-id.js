...
export default async function handler(req, res) {
  ...
  try {
    let data = await fetchWiki("id", topic);
    let languageUsed = "Indonesia";

    if (!data) {
      data = await fetchWiki("en", topic);
      languageUsed = "English (fallback)";
    }
    ...
    return res.status(200).json({
      result: {
        ...
        title: data.title,
        summary: data.extract, // sudah cukup panjang biasanya
